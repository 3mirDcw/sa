  ...
  if (cmd) {
    if (db.has(`karalist_${message.author.id}`) === true) {
      let embed = new Discord.RichEmbed()
        .setColor("RANDOM")
        .setDescription(
          "**Sen botun komutlarını kullanamazsın! Çünkü botun kara listesindesin Neden Alındıgını Öğrenmek İçin https://discord.gg/sA8uSa9 destek sunucumuza gel**"
        );
    ...
    if (db.has(`yasakK_${message.guild.id}`) === true) {
      if (db.fetch(`yasakK_${message.guild.id}`).includes(cmd.help.name))
        return message.channel.send("Bu komut bu sunucuda **yasaklanmıştır!**");
    }
