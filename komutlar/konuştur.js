...
exports.run = (client, message, args) => {
  ...
  if (!message.guild.members.get(client.user.id).hasPermission("MANAGE_ROLES"))
    return message.reply("Gerekli iznin yok!");
  //if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply(`Bu komutu kullanabilmek için **Mesajları Yönet** iznine sahip olmalısın!`);

  let user = message.mentions.users.first();
  if (db.has(`codeminglog_${message.guild.id}`) === false)
    return message.reply("Mod log kanalı ayarlanmamış");
  let modlog = message.guild.channels.get(
    db
      .fetch(`codeminglog_${message.guild.id}`)
      .replace("<#", "")
      .replace(">", "")
  );
  if (db.has(`sRol_${message.guild.id}`) === false)
    return message.reply("Sustur rolü ayarlanmamış");
  ...
  if (user.id === message.author.id)
    return message.reply("hayır sen olamazsın!");
  ...
  if (!message.guild.member(user).roles.has(muteRole.id))
    return message.reply("Bu kişi susturulmamış!");

  const embed = new Discord.RichEmbed()
    .setColor("RANDOM")
    .addField("İşlem", "Susturma Kaldırma")
