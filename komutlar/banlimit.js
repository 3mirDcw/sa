...
exports.run = async (yashinu, message, args) => {
  ...
  if (!args[0] || isNaN(args[0]))
    return message.channel.send(
      `Ban limit sayısını belirtmelisin! (Özelliği devre dışı bırakmak isterseniz **0** yazmalısınız!)`
    );
  await db.set(`banlimit_${message.guild.id}`, args[0]);
  message.reply(
    `Sunucunun ban limitini başarıyla **${args[0]}** olarak ayarladım!`
  );
};
...
exports.help = {
  name: "banlimit",
  description:
    "banlimiti koyar koyduğunuz limiti geçen kişilerin tüm yetkisini alır.",
  usage: "!banlimit Sayı",
  kategori: "yetkili"
};
