...
exports.run = async (client, message, args) => {
  ...
  request({
    url: `https://discordapp.com/api/v7/channels/${db.fetch(
      `geciciKanal_${message.guild.id}`
    )}`,
    method: "PATCH",
    json: {
      user_limit: "3"
    ...
    message.channel.send(`Geçici Oda Sistemi Sunucunuzda Aktif!  \`(Oda isimlerini değiştirmeyin, çalışmaz!)\``)
(
  );
};
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["geçiciodakurulum",'geçici-odaları-kur'],
  permLevel: 0,
  kategori: "yetkili"
};
